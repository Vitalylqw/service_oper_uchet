...
8.  Executes the front‑end unit tests found in the React project under
    ...
    run synchronously and the exit code is checked – any failures will
...
def create_test_excels(source_excel: Path, output_dir: Path) -> Dict[str, Path]:
    ...
    * **insert** – append a new deal row with a unique client name.
    * **update** – change the total revenue of the first existing deal.
    * **delete** – remove the first deal row entirely.
...
async def test_excel_parser() -> None:
    ...
    from src.domain.models import SyncType
...
async def test_change_detection() -> None:
    ...
    async with db_manager.get_async_session() as session:
        ...
        for scenario, (exp_ins, exp_upd, exp_del) in expectations.items():
            ...
            logger.info(f"✅ Scenario '{scenario}' passed with expected changes")
...
async def test_sync_orchestrator() -> None:
    ...
    async with db_manager.get_async_session() as session:
        ...
        for scenario in scenarios:
            ...
            logger.info(f"✅ Sync scenario '{scenario}' completed with expected events")
            logger.info(
