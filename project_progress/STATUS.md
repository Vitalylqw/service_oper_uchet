...
- CLI-обёртка `so-uchet` — единая точка входа (план: `plans/STAGE-CLI.md`).
...
- реализовать CLI-обёртку `so-uchet` по плану `plans/STAGE-CLI.md` (текущий подэтап: 1 — каркас + db);
...
1. реализовать CLI-обёртку `so-uchet` (Этап B, подэтапы 1–5);
