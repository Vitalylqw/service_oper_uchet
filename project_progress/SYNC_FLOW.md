...
### 2. `read_audit` сейчас не заполняется фактически

Метод `ReadModelBuilder._create_audit_entry()` сейчас делает `return` сразу после debug-логирования,
поэтому запись в таблицу `read_audit` не происходит, хотя модель и вызовы в коде существуют.
