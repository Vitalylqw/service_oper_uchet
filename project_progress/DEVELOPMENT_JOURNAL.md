...
## 2026-04-02 - Первый post-change замер full sync после внедрения search-индексов

### Цель:
Подтвердить фактическое влияние новых search-индексов и runtime-обновлений на полную
синхронизацию, а не оставлять оценку только на уровне baseline и инженерного прогноза.

### Контекст:
Ранее для поискового этапа был зафиксирован узкий baseline по production-like full sync:

- `9` прогонов;
- диапазон `623` - `739` сек;
- средняя длительность `656.53` / `656.7` сек.

Для закрытия этого риска потребовался отдельный контролируемый post-change full sync в том же
рабочем контуре через `so-uchet`.

### Выполненные действия:
1. Проверен CLI-контур `so-uchet sync full`.
2. Выполнен один полный прогон через:
   - `./.venv/bin/so-uchet --env-file ./config.env sync full --log-level INFO`
3. После завершения подтверждена запись в `sync_sessions`.
4. Сопоставлен новый результат с ранее зафиксированным baseline.
5. Обновлен `plans/STAGE-SEARCH.md` с фактическим сравнением и промежуточным выводом.

### Результат:
- session: `ea0d7701-adf4-4bce-88a2-5b48549e7e90`;
- длительность полного прогона: `767.79` сек;
- отклонение от среднего baseline `656.53` сек: `+111.26` сек (`+16.9%`);
- отклонение от верхней границы baseline `738.62` сек: `+29.17` сек (`+3.9%`);
- разбиение по фазам runtime:
  - parsing: `131.91` сек;
  - change detection: `106.25` сек;
  - database update: `529.53` сек.

### Вывод:
- рост времени синхронизации после search-индексов подтвержден;
- рост пока выглядит умеренным и остаётся приемлемым для ночного batch-сценария;
- основной дополнительный риск сосредоточен в фазе database update/read-model refresh;
- для устойчивого вывода желательно ещё `1-2` повторных замера, чтобы отделить системный рост
  от единичного пика.

### Зачем:
Чтобы завершить не только проектирование, но и первый прикладной контроль влияния поиска
на фактический sync runtime.

---

## 2026-04-02 - Фиксация baseline по времени full sync для поискового этапа

### Цель:
Зафиксировать подтвержденный baseline по полной синхронизации перед прямой оценкой влияния
новых search-индексов на ночной batch-sync.

### Контекст:
После внедрения индексов и обновления runtime-поиска требовалось понять, есть ли уже достаточно
данных для безопасной оценки влияния на sync без немедленного повторного полного прогона.

Проверка `sync_sessions` показала:

- в живой БД фактическое поле окончания называется `finished_at`, а не `completed_at`;
- исторический набор full sync неоднороден и включает короткие тестовые/служебные прогоны;
- для инженерной оценки пригоден только узкий production-like baseline последних ночных запусков.

### Выполненные действия:
1. Снята фактическая схема `sync_sessions`.
2. Проверены последние завершенные `full`-сессии и их длительность.
3. Отдельно рассчитан узкий baseline по production-like full sync за `2026-03-31` - `2026-04-02`.
4. Обновлен `plans/STAGE-SEARCH.md` с подтвержденными числами и ограничениями измерения.

### Результат:
- подтвержден baseline по `9` последним полноценным full sync:
  - диапазон: `623` - `739` сек;
  - среднее: `656.7` сек;
  - размер входного файла: около `20.1 MB`;
- зафиксировано, что точное сравнение "до/после" пока не закрыто, так как после внедрения
  поисковых индексов еще не выполнялся отдельный контролируемый full sync.

### Зачем:
Чтобы не подменять реальное измерение предположением и иметь документированный baseline
для следующего прикладного шага: одного post-change full sync в спокойное окно.
