...
class SchedulerConfig(BaseSettings):
    ...
    full_sync_schedule: str = Field(default="weekly", description="Full sync frequency (daily/weekly/monthly)")
    ...
    # Error handling
    continue_on_sync_failure: bool = Field(default=False, description="Continue scheduling after sync failure")
    alert_on_failure: bool = Field(default=True, description="Send alerts on sync failure")
    max_consecutive_failures: int = Field(default=5, description="Max consecutive failures before disabling")

    # File monitoring
    file_monitoring_enabled: bool = Field(default=True, description="Enable file change monitoring")
    file_monitoring_interval_seconds: int = Field(default=300, description="File monitoring interval")

    # Job persistence
    job_store_type: str = Field(default="memory", description="Job store type (memory/redis/postgresql)")
    job_store_url: str = Field(default="", description="Job store connection URL")

    # Windows Task Scheduler (backup/watchdog)
    windows_task_scheduler_enabled: bool = Field(default=False, description="Enable Windows Task Scheduler watchdog")
    windows_task_name: str = Field(default="ServiceOperUchet_Sync", description="Windows task name")
    python_executable_path: str = Field(default="", description="Python executable path for Windows task")
