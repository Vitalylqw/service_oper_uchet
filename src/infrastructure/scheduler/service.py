...
class SchedulerService:
    ...
    async def _schedule_sync_jobs(self) -> None:
        ...
        if self.config.daily_sync_enabled:
            ...
            self._scheduler.add_job(
                ...
                misfire_grace_time=self.config.scheduler_job_defaults.get("misfire_grace_time", 300),
                max_instances=1,
            )
        ...
        if self.config.full_sync_schedule != "manual":
            ...
            if self.config.full_sync_schedule == "weekly":
                ...
                logger.info(f"Scheduled weekly full sync on {self.config.full_sync_day} at {self.config.full_sync_time}")
    ...
    async def _execute_sync_job(self, job_type: SyncJobType, job_name: str) -> JobExecutionResult:
        ...
        try:
            ...
            async for attempt in AsyncRetrying(
                ...
                ) if self.config.retry_exponential_backoff else wait_exponential(min=self.config.retry_delay_seconds, max=self.config.retry_delay_seconds),
                ...
                with attempt:
                    ...
                    result.status = JobExecutionStatus.RETRYING if attempt.retry_state.attempt_number > 1 else JobExecutionStatus.RUNNING
                    ...
                    result.changes_detected = getattr(sync_result.change_detection_result, 'total_changes', 0) if sync_result.change_detection_result else 0
                    result.events_created = len(sync_result.events_created) if sync_result.events_created else 0
            ...
            logger.info(f"✅ Sync job {job_id} completed successfully in {result.duration_seconds:.2f}s")
            ...
            logger.error(f"❌ Sync job {job_id} failed after {self.config.max_retry_attempts} attempts: {result.error_message}")
            ...
            if result.attempt_number >= self.config.max_retry_attempts:
                result.error_message = f"Job failed after {self.config.max_retry_attempts} attempts: {str(e)}"
            else:
    ...
    def _update_success_metrics(self, result: JobExecutionResult) -> None:
        ...
        if result.duration_seconds:
            if self._metrics.total_jobs_completed == 1:
                ...
                # Update average
                total_time = self._metrics.average_execution_time_seconds * (self._metrics.total_jobs_completed - 1)
                self._metrics.average_execution_time_seconds = (total_time + result.duration_seconds) / self._metrics.total_jobs_completed

                # Update min/max
                self._metrics.min_execution_time_seconds = min(self._metrics.min_execution_time_seconds, result.duration_seconds)
                self._metrics.max_execution_time_seconds = max(self._metrics.max_execution_time_seconds, result.duration_seconds)
    ...
    def _update_failure_metrics(self, result: JobExecutionResult) -> None:
        ...
        if total_jobs > 0:
            self._metrics.failure_rate_percent = (self._metrics.total_jobs_failed / total_jobs) * 100.0
    ...
    async def _handle_job_failure(self, result: JobExecutionResult) -> None:
        ...
        if self._state.consecutive_failures >= self.config.max_consecutive_failures:
            logger.error(f"Disabling scheduler due to {self._state.consecutive_failures} consecutive failures")
    ...
    async def trigger_sync(self, job_type: SyncJobType, job_name: str = "Manual sync") -> JobExecutionResult:
    ...
    def get_scheduled_jobs(self) -> list[JobScheduleInfo]:
        ...
        for job in self._scheduler.get_jobs():
            job_info = JobScheduleInfo(
                ...
                total_executions=len([r for r in self._job_results.values() if r.job_name == job.name]),
            )
