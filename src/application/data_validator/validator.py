...
class DataValidator:
    ...
    def _validate_sheet_structure(self, excel_file: pd.ExcelFile, sheet_name: str,
    ...
    def _validate_headers(self, df: pd.DataFrame, sheet_name: str,
                         result: ValidationResult) -> None:
    ...
    def _validate_financial_consistency(self, deal: Deal, result: ValidationResult) -> None:
        ...
        calculated_revenue = sum(
            item.revenue.amount for item in deal.items
            if item.revenue
        )
        calculated_margin = sum(
            item.margin.amount for item in deal.items
            if item.margin
        )
